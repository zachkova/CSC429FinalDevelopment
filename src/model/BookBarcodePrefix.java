...
public class BookBarcodePrefix extends EntityBase {
    ...
    public BookBarcodePrefix(String barcode) throws InvalidPrimaryKeyException {
        super(myTableName);
        this.setDependencies();
        String query = "SELECT * FROM " + myTableName + " WHERE (prefixValue = " + barcode + ")";
        Vector allDataFromDB = this.getSelectQueryResult(query);
        if (allDataFromDB != null) {
            int dataLen = allDataFromDB.size();
            if (dataLen != 1) {
                throw new InvalidPrimaryKeyException("Multiple barcodes matching id : " + barcode + " found.");
            } else {
    ...
    private void updateStateInDatabase()
    {
        ...
        try
        {
            if (exists == true)
            {
                ...
                updateStatusMessage = "Book data for book barcode number : " + persistentState.getProperty("prefixValue") + " updated successfully in database!";
            }
            else
            {
